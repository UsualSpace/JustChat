...
const chat = require("./controllers/group_controller")
...
//CHAT RELATED ROUTES.
router.get("/groups", middleware.AuthenticateSession, ); //get a list of all group chats and general info that the user with the valid session is a member of.
router.get("/groups/:group-id/messages", middleware.AuthenticateSession, ); //Get all messages in a chat with.
router.post("/groups", middleware.AuthenticateSession, chat.CreateGroup); //Post a new chat.
